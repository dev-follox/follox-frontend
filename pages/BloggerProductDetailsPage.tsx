...
const BloggerProductDetailsPage: React.FC = () => {
	...
	const fetchData = useCallback(async () => {
		...
		try {
			...
			if (foundProduct) {
				...
				setError(t('bloggerProductDetails.notFound'));
			}
		} catch (err) {
			setError(t('bloggerProductDetails.loadError'));
		} finally {
			setLoading(false);
		}
	}, [productId]);
	...
	if (!product) {
		return <div className="text-center text-gray-500">{t('bloggerProductDetails.notFound')}</div>;
	}
		...
		<div className="h-full w-full p-4 md:p-8">
			<div className="mb-6">
				<Button onClick={() => navigate('/blogger/products')} variant="secondary" size="sm">
					{t('bloggerProductDetails.backToList')}
				</Button>
			</div>
			...
			<div className="flex flex-col md:flex-row gap-8 mb-6">
				...
				<div className={`${product.image_url ? 'md:w-1/2' : 'w-full'}`}>
					...
					{product.description && (
						<div className="mb-6">
							<h2 className="text-xl font-semibold text-gray-800 mb-2">{t('bloggerProductDetails.description')}</h2>
							<p className="text-gray-700 leading-relaxed">{product.description}</p>
						</div>
					)}
					{product.blogger_task_description && (
						<div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
							<h2 className="text-xl font-semibold text-blue-900 mb-2">{t('bloggerProductDetails.taskFromShop')}</h2>
			...
			<div className="mt-8 p-6 bg-gray-50 rounded-lg">
				<h2 className="text-xl font-semibold text-gray-800 mb-4">{t('bloggerProductDetails.affiliateLink')}</h2>
				<div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
					...
					<Button
						onClick={() => {
							...
							alert(t('bloggerProductDetails.linkCopied'));
						}}
						variant="secondary"
						size="sm"
					>
						{t('bloggerProductDetails.copy')}
					</Button>
				</div>
			</div>
		</div>
